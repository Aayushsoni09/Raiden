...
2. **Executor** — the only component that can mutate cloud state. Accepts
   only `(runbook_name, params)` tuples, never free-form shell text from
...
| Risk | Mitigation |
|---|---|
| LLM hallucinates a destructive command | Executor only accepts predefined runbook names + schema-validated params; no shell string ever reaches subprocess from the LLM directly. |
| LLM tries a write-capable CLI verb via investigator tools | `tools.py` allowlists only read verbs (describe/list/get/logs) per CLI and rejects everything else before subprocess runs. |
| Wrong project's resources get touched | Resolver never guesses on ambiguous matches — raises `AmbiguousMatchError` instead of picking one. |
| A runbook is quietly upgraded to do something destructive | CI (`lint_runbooks.py`) rejects `terraform apply/destroy` substrings and hardcoded-forbidden names (`any_delete`, `iam_modify`) at lint time; `executor.py` re-checks at load and run time. |
...
| Credentials committed to the repo | `catalog/*.yaml` (except schema/example) and `.env`/`*.pem`/`*.key`/`credentials/` are gitignored; CI runs `bandit` on `src/`. |
